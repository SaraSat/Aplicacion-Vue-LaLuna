        ...
        <v-layout v-for="(item,index) in items " :key="index" wrap>
            ...
            <v-flex xs12>
                <v-card dark height="100%" class="jumbotron">
                   <v-btn @click="ed=true" class="info float-right mt-4 mr-4">Editar</v-btn>
                    <v-card-title>
                      <h1 v-if="!ed">{{item.dia}}</h1>  
                      <h1 v-if="ed"><v-text-field  v-model="item.dia"></v-text-field></h1>
                      </v-card-title>
                    <v-card-subtitle>
                      <h4 v-if="!ed">{{item.fecha}} a las  {{item.hora}} en {{item.lugar}}</h4>  
                      <h4 v-if="ed"><v-text-field  v-model="item.dia"></v-text-field>a las 
                      <v-text-field  v-model="item.hora"></v-text-field> en <v-text-field  v-model="item.lugar"></v-text-field></h4>
                    </v-card-subtitle>
                    <v-card-text>
                      <p v-if="!ed">{{item.desc}}</p>
                      <p v-if="ed"><v-text-field  v-model="item.desc"></v-text-field></p>
                      <p v-if="!ed">La actividad tendrá un coste de {{item.precio}}€</p>
                      <p v-if="ed">La actividad tendrá un coste de <v-text-field  v-model="item.precio"></v-text-field> €</p>
                    </v-card-text>
                    <v-card-actions>
            ...
            <v-flex xs12>
                <v-card dark height="100%" class="jumbotron">
                    ...
                    <v-card-subtitle>
                      ...
                      <h4 v-if="ed">{{item.fecha}} a las <v-text-field  v-model="item.horaF"></v-text-field> en <v-text-field  v-model="item.lugarF"></v-text-field></h4>
                    </v-card-subtitle>
                    <v-card-actions>
                        <iframe class="d-block w-100" src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3032.1354486690466!2d-3.6343836846003565!3d40.53859657935119!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0xd422cf780c46225%3A0xc4af6e1770c0aefe!2sCasa%20de%20las%20Asociaciones!5e0!3m2!1ses!2ses!4v1586739419365!5m2!1ses!2ses"
                        width="mr-auto" height="350" frameborder="0" style="border:3;" allowfullscreen="" aria-hidden="false" tabindex="0"></iframe>
                    </v-card-actions>
                    <v-btn  class="info float-right mt-4 mr-4" v-if="ed" @click="edit">Aceptar</v-btn>
                </v-card>
            </v-flex>
        </v-layout>
